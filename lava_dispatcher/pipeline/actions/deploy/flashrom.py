...
class FlashSPI(Action):
    ...
    def run(self, connection, max_end_time, args=None):
        connection = super(FlashSPI, self).run(connection, max_end_time, args)
        params = self.job.device['actions']['deploy']['methods']['flashrom']
        coreboot_file = self.get_namespace_data(action='download_action',
                                                label='boot', key='coreboot')
        flashrom_cmd = ['flashrom', '-w', coreboot_file]
        if 'chip' in params:
            flashrom_cmd.extend(['-c', params['chip']])
        if 'driver' in params:
            flashrom_cmd.extend(['-p', params['driver']])
        self.logger.info("Start running flashrom")
...
class SPIPowerOn(Action):
    ...
    def run(self, connection, max_end_time, args=None):
        connection = super(SPIPowerOn, self).run(connection, max_end_time, args)
...
class SPIPowerOff(Action):
    ...
    def run(self, connection, max_end_time, args=None):
        connection = super(SPIPowerOff, self).run(connection, max_end_time, args)
        if not hasattr(self.job.device, 'power_state'):
            return connection
